Ext.define('Opc.controller.hinv.WoRawMaterialMapping', {
	extend : 'Opc.controller.BaseController',

	views : [ 'hinv.WoRawMaterialMapping' ],

	refs : [ {
		selector : 'hinv_wo_raw_material_mapping',
		ref : 'panel'
	} ],

	init : function() {
		this.control({
			'hinv_wo_raw_material_mapping' : {
				btnClose : this.onBtnClose
			},

			'hinv_wo_raw_material_mapping textfield[itemId=lotId]' : {
				keypress : this.onInputLotId
			},

			'hinv_wo_raw_material_mapping button[itemId=btnProcess]' : {
	...
	onProcessBtnClick :function(button ,e){
		var url = 'service/HinvWoRawMaterialMapping.json';
		...
		var qty = panel.sub('qty').getValue();
