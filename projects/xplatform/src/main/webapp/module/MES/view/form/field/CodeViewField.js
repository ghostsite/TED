 ...
 */
Ext.define('MES.view.form.field.CodeViewField', {
	...
	initComponent : function() {
		...
		this.on('disabledfield',function(state,index){
			var field = self.sub(self.fields[index].itemId);
			if(field)
	...
	buildTxtfield : function(alignIcon) {
		var codeviewOpts = SmartFactory.codeview.get(this.codeviewName)||{};
		...
		var fieldCnt = name.length==0?1:name.length;
		...
		if(fields.length == 0){
			...
			if(popupConfig.columns) {
				for(var i = 0 ; i < fieldCnt ; i++){
					...
					var dataIndex = popupConfig.columns[i].dataIndex;
					fields = [{
						column : dataIndex,
						flex : 1
					}];
		...
		for(var i in fields){
				...
				listeners : {
					specialkey : function(textfield, e) {
						...
						if(textfield.disabledEnter !== true){
							SmartFactory.codeview.show(fieldset);	
						}
					},
			...
			fields[i].itemId = this.itemId+'_'+i;
			items[index] = fields[i];
			if(fields[i].width){
				delete items[index].flex;
			}
			if(name[i])
				items[index].name = name[i];
			if(value[i])
				items[index].value = value[i];
			
			//2012.08.08 우선순위변경 필드설정>필드셋설정
			//2012.08.27 blankText 추가
	...
	selectedCallback : function(fieldset, record) {
		...
		for(var i in fields){
			var fieldId = fieldset.itemId+'_'+i;
			var field = fieldset.getComponent(fieldId);
			var oldVal = field.getValue();
			var newVal = '';
			if(record){
				newVal = record.get(fields[i].column);
			}
			//field.setRawValue(newVal); changetext 이벤트 발생안함.
		...
		if (changeCnt > 0) {
			fieldset.fireEvent('change', fieldset, newValues, oldValues, 'selected' );
		}
		fieldset.fireEvent('select', record);
	},
	...
	setValue : function(value,index) {
		...
		for(var i in this.fields){
			...
			var field = this.getComponent(fieldId);
			var oldVal = field.getValue();
			var newVal = i == index?value:oldVal;
			if(oldVal !== newVal){
		...
		if (changeCnt > 0) {
			this.fireEvent('change', this, newValues, oldValues, 'setvalue' );
		}
	},
	...
	setValues : function(values) {
		...
		for(var i in this.fields){
			...
			var newVal = '';
			if(values != '')
				newVal = values[i]==undefined?oldVal:values[i];
			if(oldVal !== newVal ){
				field.setRawValue(newVal);	
				changeCnt++;
			}
	...
	getValues : function(){
		...
		for(var i in this.fields){
			var fieldId = this.itemId+'_'+i;
			var val = '';
			if(this.getComponent(fieldId)){
				val=this.getComponent(fieldId).getValue();
			}
			values.push(val);
		}
		return values;
