Ext.define('BAS.controller.VendorSetup', {
	extend : 'Ext.app.Controller',

	stores : [ 'BAS.store.BasViewDataListOut.DataList' ],
	models : [ ],
	views : [ 'BAS.view.setup.VendorSetup','BAS.view.common.NavTaskRequest' ],
	
	refs : [ {
		selector : 'bas_vendor_setup',
		ref : 'vendorSetup'
	}, {
	...
	onKeyChange :function(view, keys){
		...
		if(keys.reqType != 'import'){
			...
			view.sub('txtVendorId').setValue(keys.key1||'');
			if(!keys.taskReqId && keys.reqType != 'update' && keys.reqType != 'delete'){
				view.sub('txtVendorId').setReadOnly(false);
			}
			else{
				view.sub('txtVendorId').setReadOnly(true);
				view.formLoad(keys);
			}
		}
	},
