...
import com.ted.common.spring.mvc.util.MapWapper;

/**
 * 
 * 解析请求参数json字符串 
 * 
 * @author Zhang Kaitao,ghostzhang
 * @since 3.1
 */
public class RequestJsonParamMethodArgumentResolver extends AbstractNamedValueMethodArgumentResolver implements WebArgumentResolver {
        ...
        try {
            if (paramValues.length == 1) {
                ...
                if (MapWapper.class.isAssignableFrom(paramType)) {
                    ...
                    MapType mapType = (MapType) getJavaType(type, HashMap.class);
                ...
                JavaType javaType = getJavaType(type, paramType);
    ...
    protected JavaType getJavaType(Type type, Class<?> contextClass) {
        return (contextClass != null) ? this.mapper.getTypeFactory().constructType(type, contextClass) : this.mapper.constructType(type);
    }
