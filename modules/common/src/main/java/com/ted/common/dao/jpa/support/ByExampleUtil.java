...
public class ByExampleUtil {
	...
	public <T extends Persistable<?>, M2O extends Persistable<?>> List<Predicate> byExampleOnXToOne(ManagedType<T> mt, Root<T> mtPath, final T mtValue,
		...
		for (SingularAttribute<? super T, ?> attr : mt.getSingularAttributes()) {
			if (attr.getPersistentAttributeType() == MANY_TO_ONE || attr.getPersistentAttributeType() == ONE_TO_ONE) { //
				...
				if (m2oValue != null && mtValue.isNew()) { //here is changed
